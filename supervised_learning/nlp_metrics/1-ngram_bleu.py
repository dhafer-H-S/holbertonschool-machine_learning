   ...
   # Create n-grams for sentence
    ...
    for reference in references:
        ...
        for ngram in sentence_ngrams:
            max_counts[ngram] = max(max_counts.get(ngram, 0), reference_ngrams[ngram])

    clipped_counts = {ngram: min(count, max_counts.get(ngram, 0)) for ngram, count in sentence_ngrams.items()}
